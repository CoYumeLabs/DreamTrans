# 历史交接记录：训练折扣/赠送额度 + 管理员后台（2026-09-07）

> 本文以下为接手前快照，不代表当前工作树状态。批量转录已在 `d5d535e` 推送，训练额度修正在 `d7b5313` 推送，两次远端 CI 均通过。后续后台实现与现行限制见 [后台运营说明](../ADMIN_OPERATIONS.md)。原文提到的旧测试、lint、主动训练弹窗、设置页路由提示、批量预扣路由不一致已处理；请勿按旧清单重复修复。
