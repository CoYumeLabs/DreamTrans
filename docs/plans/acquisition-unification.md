...
- 代理已发出但未兑换的码，迁移后按代理**当前**的面值和有效天数计价（来源只有一套条件）；
  如需保留旧面值，迁移前先把旧批次作废重发。
- 风控标记只保留能对应到归因记录的那部分：兑码时输给更早归因的账户，或账户已删除的标记会被丢弃，
  它们本来也没有可结算的分成。
- 一个账户同时有活动注册和已兑的码时，归因给时间更早的那个，另一个只在码上记"已用"。

## 行为变化（对外）

- 兑换码只能由**尚未归因**的已验证账户使用；文案改为"每个账户只能通过一种活动获得赠送"。
- 代理多了链接 `/invite?code=<代理来源码>`，通过链接注册的用户享受与码相同的赠送并归因给代理；
  链接注册人数与当天发出的码合计受每日发码限额约束，满额后链接当天关闭。代理发码截止日最多 90 天。
- 已确认的规则（2026-09-09）：注销过的邮箱带活动码再注册时整个注册失败；风控压住赠送时码仍然被消耗。
