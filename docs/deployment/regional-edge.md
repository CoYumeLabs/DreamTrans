...
- v1/v2 均与主站直连一致：按成功转发给供应商的 PCM 音频时长计费，不等待 partial、final 或 `AudioAdded`。有可信终止用量时，退回未发送的预留部分；异常断流不免除已发送音频费用，重连时实际重放给供应商的音频再次计费。最终字幕水位只决定恢复缓冲，不决定收费量。
- 已连接但缺少终止用量的过期或接管代次会被禁止继续工作，原预算保持待核对；不能根据上一次周期报告假定剩余音频都未发送。只有从未连接且没有事件证据的授权可以全额释放。可信旧节点后来提交完整终止用量时，主站按原预算和价格快照结算一次，核对额度及已知用量下界，不改变继任代次的水位和历史字幕。普通迟到 final 仅留作证据，已最终结算的旧账单不追溯重算；终止记录永久丢失时仍需人工核对。
