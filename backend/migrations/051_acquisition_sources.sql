...
ALTER TABLE promotion_registrations ADD COLUMN code_id UUID UNIQUE;
...
UPDATE redeem_codes c SET created_at=b.created_at,created_by=b.created_by,client_request_id=b.client_request_id,expires_at=b.expires_at FROM redeem_batches b WHERE b.id=c.batch_id;
...
INSERT INTO promotion_invites(code,name,channel,tags,enabled,expires_at,max_registrations,grant_usd,grant_days,plan_days,created_by,created_at,kind,owner_user_id,claim_mode)
SELECT 'AG-'||upper(substr(md5(a.user_id::text),1,10)),LEFT('代理 '||COALESCE(NULLIF(u.name,''),u.email),100),a.channel,'["agent"]'::jsonb,
...
SELECT 'RB-'||upper(replace(b.id::text,'-','')),LEFT(b.channel||' 兑换码 '||to_char(b.created_at AT TIME ZONE 'UTC','YYYY-MM-DD'),100),b.channel,b.tags,TRUE,
       b.expires_at,b.quantity,b.face_value_usd,b.grant_days,30,b.created_by,b.created_at,'campaign',NULL,'code'
...
FROM redeem_batches b JOIN promotion_invites i
  ON i.code=CASE WHEN b.agent_user_id IS NULL THEN 'RB-'||upper(replace(b.id::text,'-','')) ELSE 'AG-'||upper(substr(md5(b.agent_user_id::text),1,10)) END
...
-- Claims and referrals become attribution rows. An account attributed twice
-- keeps its first source; a redeemed code stays recorded on the code.
...
SELECT c.invite_id,c.redeemed_by,encode(sha256(convert_to(COALESCE(u.email_canonical,lower(u.email)),'UTF8')),'hex'),c.redeemed_at,c.redeemed_at,c.grant_id,c.training_opt_in_at_claim,c.id
FROM redeem_codes c JOIN users u ON u.id=c.redeemed_by
WHERE c.redeemed_by IS NOT NULL
ORDER BY c.redeemed_at
ON CONFLICT DO NOTHING;

INSERT INTO promotion_registrations(invite_id,user_id,canonical_email_hash,registered_at,rewarded_at)
SELECT i.id,r.referred_user_id,r.canonical_email_hash,r.registered_at,r.registered_at
...
-- Fraud flags follow the attribution, so link and code sign-ups are checked alike.
