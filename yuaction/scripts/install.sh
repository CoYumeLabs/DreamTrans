...
repository="CoYumeLabs/DreamTrans"
source_path="yuaction"
...
discover_dreamtrans() {
  ...
  elif [[ -f "$dreamtrans_dir/.bluegreen/state.json" ]]; then
    # Blue-green applications have no Compose labels. Use only the running
    # proxy that mounts this installation's controller-owned route directory.
    local mounts aliases found=false
    while IFS= read -r id; do
      [[ -n "$id" ]] || continue
      mounts=$(docker inspect --format '{{range .Mounts}}{{if eq .Destination "/release"}}{{println .Source}}{{end}}{{end}}' "$id")
      [[ "$mounts" == "$dreamtrans_dir/.bluegreen/proxy" ]] || continue
      aliases=$(docker inspect --format "{{with index .NetworkSettings.Networks \"$network\"}}{{range .Aliases}}{{println .}}{{end}}{{end}}" "$id")
      if [[ $'\n'"$aliases"$'\n' == *$'\n'dreamtrans$'\n'* ]]; then
        found=true
        break
      fi
    done < <(docker ps -q --filter "network=$network")
    $found || die "未找到此蓝绿主站的固定入口；请先运行 dreamtransctl sync-entry"
    set_env_value YUFOLO_URL "http://dreamtrans:8080" "$work_dir/.env"
  else
    log "尚未发现 DreamTrans 应用容器；保留已有 YUFOLO_URL，首次安装需配置主站地址"
  fi
}
...
validate_config() {
  ...
  prefix=$(env_value IMAGE_PREFIX "$file"); prefix="${prefix:-ghcr.io/coyumelabs/dreamtrans-yuaction}"
...
ENV
  ...
  prefix=$(env_value IMAGE_PREFIX "$work_dir/.env"); prefix="${prefix:-ghcr.io/coyumelabs/dreamtrans-yuaction}"

  # Existing official installs move to the monorepo package without changing
  # their Compose project, database, port, keys or configured custom registries.
  if [[ "$prefix" == ghcr.io/coyumelabs/yuaction ]]; then
    prefix=ghcr.io/coyumelabs/dreamtrans-yuaction
  fi
  set_env_value IMAGE_PREFIX "$prefix" "$work_dir/.env"
  ...
  fetch "https://raw.githubusercontent.com/$repository/$revision/$source_path/$compose_source" "$work_dir/compose.ghcr.yml"
  fetch "https://raw.githubusercontent.com/$repository/$revision/$source_path/scripts/install.sh" "$work_dir/install.sh"
