...
try {
  const authorization = (generation: number, previousAudio = 0): EdgeAuthorization => ({
    ...
    grant: { session_id: 'session', generation, previous_generation: generation - 1,
      previous_audio_sequence: previousAudio, sample_rate: 44100 },
  })
  ...
  retry.close()
  const third = new RegionalEdgeSocket(authorization(3), buffer)
  ...
  assert.equal(new DataView(replay.buffer).getBigUint64(0), 1n)
  assert.deepEqual([...new Float32Array(replay.buffer.slice(8))], [3, 4])
  thirdNative.message({ message: 'EdgeSaved', generation: 3, audio_sequence: 1 })
  ...
  new RegionalEdgeSocket(authorization(4), buffer)
