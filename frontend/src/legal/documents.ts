      ...
      blocks: [
        {
          ...
          items: [
            ...
            '促销、邀请或活动赠送的额度不适用训练计划折扣，按标准价计费，并经未开启训练的账户处理。充值余额（含充值赠送部分）按你的训练计划选择计费。赠送额度先于充值余额扣减；一场会话中赠送额度用完时，该场会话保持原有账户与价格，结束后按你当时的选择将付费部分多收的差额退回钱包并记入账单。训练计划是可选项目，我们可能暂停或重新开放；暂停后所有已加入用户自动退出，重新开放时需重新选择加入；变更只影响之后的会话。机构账户一律经未开启训练的账户处理。',
